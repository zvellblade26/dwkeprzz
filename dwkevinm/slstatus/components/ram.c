...
#include <stdio.h>
#include <math.h> // Include this for round function
...
#include "../util.h"
	...
	ram_total(const char *unused)
	{
		...
		double total_gb = total / 1024.0 / 1024.0; // Convert to GiB
		return bprintf("%.1f", total_gb); // Format to one decimal place
	}

	const char *
	ram_used(const char *unused)
	{
		uintmax_t total, available, used;
		...
		used = total - available; // Calculate used in kB based on MemAvailable
		double used_gb = used / 1024.0 / 1024.0; // Convert to GiB
		double used_percentage = (used / (double)total) * 100.0; // Calculate percentage
		return bprintf("%.2f%%  %.2fG", used_percentage, used_gb); // Format both values
	}
