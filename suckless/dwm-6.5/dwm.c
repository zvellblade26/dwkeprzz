...
enum { SchemeNorm, SchemeSel, SchemeTitle, SchemeStatus, SchemeUrg }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if(!(occ & 1 << i || m->tagset[m->seltags] & 1 << i))
			continue;
		w = TEXTW(tags[i]);
		drw_setscheme(drw, scheme[urg & 1 << i ? SchemeUrg : m->tagset[m->seltags] & 1 << i ? SchemeSel : SchemeNorm]); /* Urgency tag */
		drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], 0);
		x += w;
	}
	w = TEXTW(m->ltsymbol);
	drw_setscheme(drw, scheme[SchemeNorm]);
	//x = drw_text(drw, x, 0, w, bh, lrpad / 2, m->ltsymbol, 0); /* Layout symbol on bar */
